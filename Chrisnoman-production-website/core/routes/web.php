...
Route::get('/gown-builder', function () {
    return Inertia::render('GownBuilder');
})->name('gown-builder');

Route::get('/track-order', function (Illuminate\Http\Request $request) {
    $booking = null;
    $error = null;
    if ($request->has('booking_id') && $request->has('contact')) {
        // Strip non-numeric characters from contact to match easily if phone
        $contactVal = trim($request->contact);
        $booking = \App\Models\Booking::where('id', $request->booking_id)
            ->where(function($query) use ($contactVal) {
                $query->where('phone', 'like', "%{$contactVal}%")
                      ->orWhere('email', 'like', "%{$contactVal}%");
            })->first();
            
        if (!$booking) {
            $error = "No booking matching this Reference ID and Contact number was found.";
        }
    }
    return Inertia::render('TrackOrder', [
        'booking' => $booking,
        'error' => $error,
        'searched' => $request->has('booking_id'),
    ]);
})->name('track-order');
