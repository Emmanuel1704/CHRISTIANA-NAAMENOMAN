                                                ...
                                                <div>
                                                    ...
                                                    <div className="mt-1">
                                                        <span className={`inline-flex items-center rounded-full px-2 py-0.5 text-[9px] font-bold uppercase tracking-wider ${
                                                            item.stock_status === 'out_of_stock' ? 'bg-red-50 text-red-700 border border-red-100' :
                                                            item.stock_status === 'custom_only' ? 'bg-[#DCA73A]/10 text-[#DCA73A] border border-[#DCA73A]/20' :
                                                            'bg-green-50 text-green-700 border border-green-100'
                                                        }`}>
                                                            {item.stock_status === 'out_of_stock' ? 'Sold Out' :
                                                             item.stock_status === 'custom_only' ? 'Custom sewing only' : 'In Stock'}
                                                        </span>
                                                    </div>
                                                </div>
                                                <span className="font-bold text-[#DCA73A]">{item.currency === 'USD' ? '$' : item.currency === 'EUR' ? '€' : item.currency === 'GBP' ? '£' : 'GH₵'}{item.price ? Number(item.price).toLocaleString() : '0.00'}</span>
                                            </div>
